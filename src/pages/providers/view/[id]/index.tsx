...
import { deleteHospitalById } from 'apiSdk/hospitals';
...
function ProviderViewPage() {
  ...
  const hospitalHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('doctor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/doctors/create?provider_id=${data?.id}`}>
              ...
              {hasAccess('hospital', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/hospitals/create?provider_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.hospital?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/hospitals/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              ...
                              {hasAccess('hospital', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    hospitalHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
